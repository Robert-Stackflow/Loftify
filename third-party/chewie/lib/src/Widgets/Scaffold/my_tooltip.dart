...
/// other user interface action. Wrap the button in a [MyTooltip] widget and provide
/// a message which will be shown when the widget is long pressed.
///
/// Many widgets, such as [IconButton], [FloatingActionButton], and
/// [PopupMenuButton] have a `tooltip` property that, when non-null, causes the
/// widget to include a [MyTooltip] in its build.
///
...
///
/// {@tool dartpad}
/// This example show a basic [MyTooltip] which has a [Text] as child.
...
/// * [TooltipTheme.data]
///
/// or it can be set directly on each tooltip with [MyTooltip.preferBelow].
///
...
/// `preferBelow` is true; the tooltip will prefer showing below [MyTooltip]'s child widget.
...
///
/// {@tool dartpad}
/// This example shows a rich [MyTooltip] that specifies the [richMessage]
...
///
/// {@tool dartpad}
/// This example shows how [MyTooltip] can be shown manually with [TooltipTriggerMode.manual]
/// by calling the [MyTooltipState.ensureTooltipVisible] function.
///
...
class MyTooltip extends StatefulWidget {
  ...
  /// [MyTooltip.message] if non-null, or the plain text value of
  /// [MyTooltip.richMessage] otherwise. Set this property to true if the app is
  ...
  static final List<MyTooltipState> _openedTooltips = <MyTooltipState>[];
  ...
  static bool dismissAllToolTips() {
    if (_openedTooltips.isNotEmpty) {
      ...
      final List<MyTooltipState> openedTooltips = _openedTooltips.toList();
      for (final MyTooltipState state in openedTooltips) {
  ...
  @override
  State<MyTooltip> createState() => MyTooltipState();
...
/// Contains the state for a [MyTooltip].
///
...
class MyTooltipState extends State<MyTooltip> with TickerProviderStateMixin {
  ...
  ///
  /// This value will either come from [MyTooltip.message] or [MyTooltip.richMessage].
  ...
  void _handleStatusChanged(AnimationStatus status) {
    ...
    switch ((_animationStatus.isDismissed, status.isDismissed)) {
      ...
      case (true, false):
        ...
        MyTooltip._openedTooltips.add(this);
  ...
  void _handleMouseEnter(PointerEnterEvent event) {
    ...
    final List<MyTooltipState> tooltipsToDismiss = MyTooltip._openedTooltips
        .where((MyTooltipState tooltip) =>
            tooltip._activeHoveringPointerDevices.isEmpty)
        .toList();
    for (final MyTooltipState tooltip in tooltipsToDismiss) {
  ...
  /// [MyTooltip.dismissAllToolTips] is called.
  ///
  ...
  Widget _buildTooltipOverlay(BuildContext context) {
    ...
    final _TooltipOverlay overlayChild = _TooltipOverlay(
      ...
      position: widget.position, // 传递 position 参数
    );
    ...
    switch (position) {
      case TooltipPosition.top:
        return Offset(
          ...
          target.dy - childSize.height - verticalOffset,
        );
      ...
      case TooltipPosition.left:
        return Offset(
          target.dx - childSize.width - verticalOffset,
          target.dy - childSize.height / 2,
        );
      case TooltipPosition.right:
        return Offset(
          target.dx + verticalOffset,
          target.dy - childSize.height / 2,
        );
    }
  }
...
class _TooltipOverlay extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    var mDecoration = decoration ??
        BoxDecoration(
          color: Colors.black87,
          borderRadius: BorderRadius.circular(4),
        );
    var color = mDecoration is BoxDecoration
        ? mDecoration.color ?? Colors.black87
        : Colors.black87;
    Widget tooltipContent = FadeTransition(
      opacity: animation,
      child: Material(
        ...
        child: Row(
          ...
          children: [
            if (position == TooltipPosition.right)
              _buildArrow(color, width: 8, height: 12),
            Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                if (position == TooltipPosition.bottom) _buildArrow(color),
                Container(
                ...
                if (position == TooltipPosition.top) _buildArrow(color),
              ],
            ),
    ...
    if (onEnter != null || onExit != null) {
      tooltipContent = _ExclusiveMouseRegion(
        onEnter: onEnter,
        onExit: onExit,
...
class _ArrowPainter extends CustomPainter {
  ...
  _ArrowPainter(this.position, this.fillColor, this.borderColor);
